...
export default function CountriesPage({ countries }) {
  ...
  const featured = safeCountries.filter(c => c.is_featured)
  const others = safeCountries.filter(c => !c.is_featured)
  ...
  return (
    ...
    >
      <div className="bg-gray-50 border-b py-10">
        <div className="max-w-5xl mx-auto px-4 text-center">
          ...
          <p className="text-gray-500 text-lg">
            Explore scholarship opportunities in {safeCountries.length} countries worldwide
          </p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-12">
        {featured.length > 0 && (
          <div className="mb-12">
            <h2 className="font-display text-2xl font-bold text-gray-900 mb-6">🌟 Top Destinations</h2>
              ...
              {featured.map(country => (
                <Link key={country.id} href={`/countries/${country.slug}`}
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h3 className="font-display font-bold text-xl text-gray-900 group-hover:text-green-600 transition-colors">
                        {country.name}
                      </h3>
        ...
        {others.length > 0 && (
          <div>
            <h2 className="font-display text-2xl font-bold text-gray-900 mb-6">More Countries</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {others.map(country => (
                <Link key={country.id} href={`/countries/${country.slug}`}
                  className="card p-4 flex items-center gap-3 group hover:shadow-md transition-shadow">
                  <span className="text-3xl">{country.flag_emoji}</span>
                  <div>
                    <div className="font-medium text-gray-800 text-sm group-hover:text-green-600 transition-colors">
                      {country.name}
                    </div>
